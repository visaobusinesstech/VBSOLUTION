...
export function EmailSettingsForm({ onSave }: EmailSettingsFormProps) {
  ...
  // Atualizar dados do formulário quando as configurações mudarem
  useEffect(() => {
    if (settings) {
      setFormData({
        smtp_host: settings.smtp_host || '',
        email_porta: settings.email_porta || 587,
        email_usuario: settings.email_usuario || '',
        smtp_pass: '', // Nunca preencher senha por segurança
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => setFormData({ ...formData, email_porta: e.target.value ? parseInt(e.target.value) : 587 })}
                />
              </div>
            </div>
